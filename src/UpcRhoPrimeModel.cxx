...
double D2SigmaDMdy(Workspace& ws, double M, double y, double dM, double dy, int iy_bin,
  ...
  auto eval_b = [&](double b, double db_step) {
    ...
    for (int ipx = 0; ipx < ws.n_bins_total; ++ipx) {
      ...
      for (int ipy = 0; ipy < ws.n_bins_total; ++ipy) {
        ...
        const double sig_local = Ax2 + Ay2;
        const int idx = ipx * ws.n_bins_total + ipy;
        tl.hist_sigma[iy_bin][idx] += sig_local * d2sig_factor;
        tl.hist_Ax2[iy_bin][idx] += Ax2 * d2sig_factor;
        tl.hist_Ay2[iy_bin][idx] += Ay2 * d2sig_factor;
        tl.hist_ReAxAy[iy_bin][idx] += AxAy.real() * d2sig_factor;
        tl.hist_ImAxAy[iy_bin][idx] += AxAy.imag() * d2sig_factor;
        sum_momentum_raw += sig_local;
      }
    }
...
#pragma omp parallel for collapse(2)
  ...
  for (int i = 1; i <= ws.cfg.n_mass_bins; ++i) {
    for (int j = 1; j <= ws.cfg.n_rapidity_bins; ++j) {
      ...
      const double val = D2SigmaDMdy(ws, M, y, dM, dy, j - 1, *thread_data[tid]);
#ifdef _OPENMP
#pragma omp critical
#endif
      {
