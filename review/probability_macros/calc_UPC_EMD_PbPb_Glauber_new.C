/*
 * calc_UPC_EMD_PbPb_Glauber_Fixed.C
 * 终极修正版：
 * 1. 严格全量引入 Pb+Pb 的 1n ~ 10n 分部截面数组，精确还原 ratio_1n 逻辑！
 * 2. 完美结合 Glauber P_surv(b) 和 10^9 fm 宏观长程积分。
 * 3. 独立保存 P_surv、纯 EMD 概率以及它们的物理乘积，方便通用读取。
 */
...
public:
    ...
    GlauberCalc(int A_ = 208, double R_ = 6.62, double a_ = 0.546, double sig_mb = 68.0) {
    ...
    void Init() {
        ...
        for (double b = 0; b <= 25.0; b += db) {
            double val_TAA = 0;
            double s_step = 0.1, phi_step = pi / 50.0; 
...
public:
    ...
    EMDCalculator(int Z_in = 82, int A_in = 208) {
        Znu = Z_in; Anu = A_in;
        InitCrossSections();
    }

    void InitCrossSections() {
        double neutronSepThr = 7.4; 
        double eStepLowE = 0.05;    
        double BrtWgnr_xsection = 640.0; 
        double BrtWgnr_width = 4.05;
        double BrtWgnr_mean = 13.42;
        ...
        for (int i = 0; i < nStepsLowE; i++) {
            ...
            temp_sig_Xn.push_back(0.93 * BrtWgnr_Con * E * E / (pow(BrtWgnr_mean * BrtWgnr_mean - E * E, 2) + E * E * BrtWgnr_width * BrtWgnr_width));
        }
...
void calc_UPC_EMD_PbPb_Glauber_new(double sqrt_s_NN_c = 5360.0,
                                   double sigma_NN_mb = 68.0,
                                   const char* outputFileName = "") {
    int Z_Pb = 82; int A_Pb = 208;
    double R_Pb = 6.62, a_Pb = 0.546; 
    
    double gamma_c = sqrt_s_NN_c / (2.0 * m_N);
    double Gamma_target_frame = 2.0 * gamma_c * gamma_c - 1.0; 
    
    EMDCalculator emdCalc(Z_Pb, A_Pb);
    GlauberCalc glauber(A_Pb, R_Pb, a_Pb, sigma_NN_mb);

    cout << "=========================================================" << endl;
    cout << " UPC EMD Calculator - Pb+Pb @ " << sqrt_s_NN_c / 1000.0
         << " TeV [FINAL PERFECT]" << endl;
    cout << " sigma_NN = " << sigma_NN_mb << " mb" << endl;
    cout << "=========================================================" << endl;

    int n_bins_b_lin = 20000;
    double b_min_lin = 12.0, b_max_lin = 1000.0; 
    double db_lin = (b_max_lin - b_min_lin) / n_bins_b_lin;

    TString outName = outputFileName;
    if (outName.IsNull()) {
        outName = Form("UPC_Probabilities_PbPb_%.0fGeV_Glauber_Final.root", sqrt_s_NN_c);
    }
